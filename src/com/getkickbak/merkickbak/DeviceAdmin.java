...
import com.getkickbak.merkickbak.R;
...
public class DeviceAdmin extends PreferenceActivity
{
	      ...
	      OnPreferenceClickListener
	{
		...
		protected void reloadSummaries()
		{
			if (mSetPassword != null)
			{
				if (mAdminActive)
				{
					...
					mSetPassword.setSummary(sufficient ? R.string.password_sufficient : R.string.password_insufficient);
				}
				else
				{
		...
		{
			if (alertIfMonkey(mActivity, R.string.monkey_reset_password)) { return; }
	...
	public static class GeneralFragment extends AdminFragment implements OnPreferenceChangeListener
	{
		...
		protected void reloadSummaries()
		{
			...
			int disabled = createKeyguardDisabledFlag();
	...
	public static class QualityFragment extends AdminFragment implements OnPreferenceChangeListener
	{
		...
		final static int[]         mPasswordQualityValues       = new int[] { DevicePolicyManager.PASSWORD_QUALITY_UNSPECIFIED,
		...
		public boolean onPreferenceChange(Preference preference, Object newValue)
		{
			...
			if (TextUtils.isEmpty(valueString)) { return false; }
			int value = 0;
			try
			{
		...
		private String qualityValueToString(int quality)
		{
			for (int i = 0; i < mPasswordQualityValues.length; i++)
			{
				...
				{
					String[] qualities = mActivity.getResources().getStringArray(R.array.password_qualities);
					return qualities[i];
				}
			}
	      ...
	      OnPreferenceClickListener
	{
		...
		public boolean onPreferenceClick(Preference preference)
		{
			if (super.onPreferenceClick(preference)) { return true; }
	      ...
	      OnPreferenceClickListener
	{
		...
		public boolean onPreferenceClick(Preference preference)
		{
			...
			{
				if (alertIfMonkey(mActivity, R.string.monkey_lock_screen)) { return true; }
				mDPM.lockNow();
				return true;
			}
		...
		{
			final DeviceAdmin activity = mActivity;
				...
				public void onClick(DialogInterface dialog, int which)
				{
					...
					builder.setPositiveButton(R.string.wipe_warning_second_ok, new DialogInterface.OnClickListener()
					{
						...
						public void onClick(DialogInterface dialog, int which)
						{
							...
							if (stillActive)
							{
								mDPM.wipeData(wipeAllData ? DevicePolicyManager.WIPE_EXTERNAL_STORAGE : 0);
							}
						}
					});
	      ...
	      OnPreferenceClickListener
	{
		...
		public boolean onPreferenceClick(Preference preference)
		{
			...
			{
				if (alertIfMonkey(mActivity, R.string.monkey_encryption)) { return true; }
	...
	public static class Receiver extends DeviceAdminReceiver
	{
		...
		public void onPasswordExpiring(Context context, Intent intent)
		{
			...
			long expr = dpm.getPasswordExpiration(new ComponentName(context, Receiver.class));
